...
static unsigned long getMsTicks() {
  ...
  //gettimeofday(&tv, &tz);
  unsigned long now = get_time() / 1000;
  //convert to ms
...
{
  unsigned long thistimereply;
  thistimereply = ((get_time() * TICRATE) / 1000000);
...
int I_Open(const char *wad, int flags) {
	...
	fds[x].file = malloc(sizeof(FIL));
	FRESULT r=f_open(fds[x].file, fname,FA_READ);
	if(r){free(fds[x].file);fds[x].file=NULL;}
	if(fds[x].file) 
	{
		fds[x].offset=0;
...
//	lprintf(LO_INFO, "I_Lseek: Seeking %d.\n", (int)offset);
	if (whence==SEEK_SET) {
		...
		f_lseek(fds[ifd].file, offset);
	} else if (whence==SEEK_CUR) {
		fds[ifd].offset+=offset;
		f_lseek(fds[ifd].file,   f_tell(fds[ifd].file) +offset);
...
//	lprintf(LO_INFO, "I_Mmap: ifd %d, length: %d, offset: %d\n", ifd, (int)length, (int)offset);
	...
	if(retaddr)
	{
		...
		lprintf(LO_ERROR, "I_Mmap: Can't mmap offset: %d (len=%ld)!\n", (int)offset, length);
		return NULL;
	}
...
void I_Read(int ifd, void* vbuf, size_t sz)
{
    ...
    for(int i = 0; i < 20; i++)
	{
	  f_read(fds[ifd].file,vbuf, sz, &readBytes);
		if( readBytes == sz)//(int)sz)
		{
			return;
		}	
